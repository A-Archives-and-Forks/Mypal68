  ...
  static observe(aSubject, aTopic, aData) {
    switch (aTopic) {
      ...
      case "recording-device-events":
        updateIndicators(aSubject, aTopic, aData, this.suppressNotifications);
        break;
...
function updateIndicators(aSubject, aTopic, aData, aSupNot) {
  if (
  ...
  if (actor) {
    ...
    tabState.suppressNotifications = aSupNot;
