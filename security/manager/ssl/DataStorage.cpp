...
void DataStorage::SetCachedStorageEntries(
    const InfallibleTArray<mozilla::psm::DataStorageEntry>& aEntries) {
...
nsresult DataStorage::Init(const InfallibleTArray<DataStorageItem>* aItems,
...
void DataStorage::ReadAllFromTable(
    DataStorageType aType, InfallibleTArray<DataStorageItem>* aItems,
    const AutoLock& aProofOfLock) {
...
void DataStorage::GetAll(InfallibleTArray<DataStorageItem>* aItems) {
  WaitForReady();
  AutoLock lock(mMutex);
