 ...
 * ~~~~~~~~~~~~~~
 *
 * CookieJarSettings is a snapshot of cookie policy and cookie permissions in a
 ...
 *
 * SharedWorkers and ServiceWorkers have their own CookieJarSettings because they
 * don't have a single parent document (SharedWorkers could have more than one,
 * ServiceWorkers have none).
 *
 * In Chrome code, we have a new CookieJarSettings when we download resources via
 * 'Save-as...' and we also have a new CookieJarSettings for favicon downloading.
 *
 * Content-scripts WebExtensions also have their own CookieJarSettings because they
 ...
 * (CookieJarSettings::CreateBlockingAll()) by forcing BEHAVIOR_REJECT as policy.
 ...
 * Initially the list is empty, but when CookieJarSettings::CookiePermission() is
 ...
 * nsILoadInfo::GetCookieJarSettings(), from Document::CookieJarSettings() and from
 * the WorkerPrivate::CookieJarSettings().
 *
 * CookieJarSettings is thread-safe, but the permission list must be touched only
 * on the main-thread.
 *
 * Testing
 * ~~~~~~~
 ...
 * need to workaround CookieJarSettings. This can be done opening a new window and
