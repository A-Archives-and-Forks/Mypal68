...
nsresult PermissionManager::CommonTestPermissionInternal(
...
#ifdef DEBUG
  {
    ...
    if (!prin) {
      if (aURI) {
        prin = BasePrincipal::CreateCodebasePrincipal(aURI, OriginAttributes());
      }
    }
    MOZ_ASSERT(prin);
...
nsresult PermissionManager::Read(const Monitor2AutoLock& aProofOfLock) {
  ENSURE_NOT_CHILD_PROCESS;
...
void PermissionManager::ConsumeDefaultsInputStream(
    nsIInputStream* aInputStream, const Monitor2AutoLock& aProofOfLock) {
  MOZ_ASSERT(!NS_IsMainThread());
